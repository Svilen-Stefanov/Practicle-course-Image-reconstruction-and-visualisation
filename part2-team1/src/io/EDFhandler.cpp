...
#ifndef EDF_HANDLER_NORMALIZE
// 1: the content of the volume will be normalized to [0,1]
// 0: disable normalization
#define EDF_HANDLER_NORMALIZE 1
#endif

const char* EDFHandler::TYPE_FLOAT = "FloatValue";

Volume_ptr EDFHandler::read(const std::string& filename)
{
    // open the file
	...
	float factor = (float) (1.0 / (maxValue - minValue + std::numeric_limits<float>::epsilon())); //epsilon to prevent divide by zero
	for (int i = 0; i < data.size(); ++i)
	{
		float d = data[i];
		float v = (d - minValue) * factor;
		assert(v >= 0);
		assert(v <= 1);
		data[i] = v;
	}
	...
	Volume_ptr vol = std::make_shared<Volume>(lowerLeft, upperRight, dimSpacing, dimSizes);
    vol->setContent(data);
...
void EDFHandler::write(const std::string& filename, const Volume_ptr vol)
{
    // open the EDF file
